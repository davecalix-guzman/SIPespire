$(document).ready(function() {
    ...
    $("#sincos").bind("plothover",
        function(event, pos, item) {
            $("#x").text(pos.x.toFixed(2));
            $("#y").text(pos.y.toFixed(2));
        });

    $("#sincos").bind("plotclick",
        function(event, pos, item) {
            if (item) {
    ...
    $.plot($("#multiple"),
        ...
        ],
        {
            ...
            xaxis: { tickColor: "rgba(0, 0, 0, 0.06)", font: { color: "rgba(0, 0, 0, 0.4)" } },
            colors: [
                Utility.getBrandColor("midnightblue"), Utility.getBrandColor("danger"), Utility.getBrandColor("indigo"),
                Utility.getBrandColor("inverse"), Utility.getBrandColor("inverse"),
                Utility.getBrandColor("midnightblue")
            ],
            tooltip: true,
            tooltipOpts: {
                content: "x: %x, y: %y"
    ...
    var variance = $.plot($("#ordered"),
        dos,
        {
            ...
            colors: [
                Utility.getBrandColor("info"), Utility.getBrandColor("inverse"), Utility.getBrandColor("midnightblue")
            ],
            tooltip: true,
            tooltipOpts: {
                content: "x: %x, y: %y"
    ...
    function plotWithOptions() {
        $.plot("#stacking",
            [d1, d2, d3],
            {
                ...
                xaxis: { tickColor: "rgba(0, 0, 0, 0.06)", font: { color: "rgba(0, 0, 0, 0.4)" } },
                colors: [
                    Utility.getBrandColor("midnightblue"), Utility.getBrandColor("info"),
                    Utility.getBrandColor("success")
                ],
                tooltip: true,
                tooltipOpts: {
                    content: "X: %x | Y: %y"
