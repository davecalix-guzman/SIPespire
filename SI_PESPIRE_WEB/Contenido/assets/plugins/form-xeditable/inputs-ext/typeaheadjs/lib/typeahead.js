...
(function($) {
    ...
    var Transport = function() {
        ...
        function Transport(o) {
            ...
            maxPendingRequests =
                utils.isNumber(o.maxParallelRequests) ? o.maxParallelRequests : maxPendingRequests || 6;
            this.url = o.url;
            ...
            this._get = (/^throttle$/i.test(o.rateLimitFn) ? utils.throttle : utils.debounce)(this._get,
                o.rateLimitWait || 300);
        }
        ...
        utils.mixin(Dataset.prototype,
            {
                ...
                _transformDatum: function(datum) {
                    ...
                    item.tokens = utils.map(item.tokens,
                        function(token) {
                ...
                _mergeProcessedData: function(processedData) {
                    ...
                    utils.each(processedData.adjacencyList,
                        function(character, adjacency) {
                            var masterAdjacency = that.adjacencyList[character];
                            that.adjacencyList[character] =
                                masterAdjacency ? masterAdjacency.concat(adjacency) : adjacency;
                        });
                },
                _getLocalSuggestions: function(terms) {
                    ...
                    utils.each(shortestList,
                        function(i, id) {
                            var item = that.itemHash[id], isCandidate, isMatch;
                            isCandidate = utils.every(lists,
                                function(list) {
                    ...
                    function processRemoteData(data) {
                        ...
                        utils.each(data,
                            function(i, datum) {
                                ...
                                isDuplicate = utils.some(suggestions,
                                    function(suggestion) {
    ...
    var InputView = function() {
        function InputView(o) {
            ...
            this.$hint = $(o.hint);
            this.$input = $(o.input).on("blur.tt", this._handleBlur).on("focus.tt", this._handleFocus)
        ...
        utils.mixin(InputView.prototype,
            EventTarget,
            {
    ...
    var DropdownView = function() {
        ...
        function DropdownView(o) {
            ...
            this.$menu = $(o.menu).on("mouseenter.tt", this._handleMouseenter)
                .on("mouseleave.tt", this._handleMouseleave).on("click.tt", ".tt-suggestion", this._handleSelection)
                .on("mouseover.tt", ".tt-suggestion", this._handleMouseover);
        }

        utils.mixin(DropdownView.prototype,
            EventTarget,
            {
                ...
                renderSuggestions: function(dataset, suggestions) {
                    ...
                    if ($dataset.length === 0) {
                        ...
                        $dataset = $("<div></div>").addClass(datasetClassName).append(dataset.header)
                            .append($suggestionsList).append(dataset.footer).appendTo(this.$menu);
                    }
    ...
    var TypeaheadView = function() {
        ...
        function TypeaheadView(o) {
            ...
            this.dropdownView = new DropdownView({
                    menu: $menu
                }).on("suggestionSelected", this._handleSelection).on("cursorMoved", this._clearHint)
                .on("cursorMoved", this._setInputValueToSuggestionUnderCursor)
                .on("cursorRemoved", this._setInputValueToQuery).on("cursorRemoved", this._updateHint)
                .on("suggestionsRendered", this._updateHint).on("opened", this._updateHint)
            ...
            this.inputView = new InputView({
                ...
                .on("blured", this._setInputValueToQuery).on("enterKeyed tabKeyed", this._handleSelection)
                .on("queryChanged", this._clearHint).on("queryChanged", this._clearSuggestions)
                .on("queryChanged", this._getSuggestions).on("whitespaceChanged", this._updateHint)
                .on("queryChanged whitespaceChanged", this._openDropdown)
                .on("queryChanged whitespaceChanged", this._setLanguageDirection).on("escKeyed", this._closeDropdown)
                .on("escKeyed", this._setInputValueToQuery).on("tabKeyed upKeyed downKeyed", this._managePreventDefault)
                .on("upKeyed downKeyed", this._moveDropdownCursor).on("upKeyed downKeyed", this._openDropdown)
        ...
        utils.mixin(TypeaheadView.prototype,
            EventTarget,
            {
                ...
                _getSuggestions: function() {
                    ...
                    utils.each(this.datasets,
                        function(i, dataset) {
                            dataset.getSuggestions(query,
    ...
    (function() {
        ...
        methods = {
            initialize: function(datasetDefs) {
                ...
                datasets = utils.map(datasetDefs,
                    function(o) {
