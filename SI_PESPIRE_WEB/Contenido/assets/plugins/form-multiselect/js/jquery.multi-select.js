    ...
    MultiSelect.prototype = {
        ...
        init: function() {
            ...
            if (ms.next(".ms-container").length === 0) {
                ...
                that.$selectableUl.on(action,
                    ".ms-elem-selectable",
                    function() {
                        that.select($(this).data("ms-value"));
                    });
                that.$selectionUl.on(action,
                    ".ms-elem-selection",
                    function() {
            ...
            if ($optgroup.length > 0) {
                ...
                if ($selectableOptgroup.length === 0) {
                    var optgroupContainerTpl = '<li class="ms-optgroup-container"></li>',
                        optgroupTpl = '<ul class="ms-optgroup"><li class="ms-optgroup-label"><span>' +
                            optgroupLabel +
                            "</span></li></ul>";
                    ...
                    if (that.options.selectableOptgroup) {
                        $selectableOptgroup.find(".ms-optgroup-label").on("click",
                            function() {
                                var values = $optgroup.children(":not(:selected)")
                                    .map(function() { return $(this).val() }).get();
                                that.select(values);
                            });
                        $selectionOptgroup.find(".ms-optgroup-label").on("click",
                            function() {
                                var values = $optgroup.children(":selected").map(function() { return $(this).val() })
                                    .get();
        ...
        'select_all': function() {
            ...
            this.$selectableUl.find(".ms-elem-selectable").filter(":not(." + this.options.disabledClass + ")")
                .addClass("ms-selected").hide();
            this.$selectionUl.find(".ms-optgroup-label").show();
            this.$selectableUl.find(".ms-optgroup-label").hide();
            this.$selectionUl.find(".ms-elem-selection").filter(":not(." + this.options.disabledClass + ")")
                .addClass("ms-selected").show();
            this.$selectionUl.focus();
            ms.trigger("change");
            if (typeof this.options.afterSelect === "function") {
                var selectedValues = $.grep(ms.val(),
                    function(item) {
