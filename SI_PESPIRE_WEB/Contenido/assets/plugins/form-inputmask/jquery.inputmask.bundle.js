    ...
    if ($.fn.inputmask == undefined) {
        $.inputmask = {
            ...
            defaults: {
                ...
                ignorables: [
                    9, 13, 19, 27, 33, 34, 35, 36, 37, 38, 39, 40, 45, 46, 93, 112, 113, 114, 115, 116, 117, 118, 119,
                    120, 121, 122, 123
                ],
            ...
            function maskScope(masksets, activeMasksetIndex) {
                ...
                function isValid(pos, c, strict) { //strict true ~ no correction or autofill
                    ...
                    if (strict) {
                        var result =
                            _isValid(pos,
                                getActiveMaskSet()); //only check validity in current mask when validating strict
                        if (result === true) {
                        ...
                        function(index, value) {
                            if (typeof (value) == "object") {
                                ...
                                if (currentActiveMasksetIndex != activeMasksetIndex && !isMask(pos)) {
                                    if (c == getActiveBufferTemplate()[maskPos] || c == opts.skipOptionalPartCharacter
                                    ) { //match non-mask item
                                        results.push({
                                            "activeMasksetIndex": index,
                                            "result": { "refresh": true, c: getActiveBufferTemplate()[maskPos] }
                                        }); //new command hack only rewrite buffer
                                        ...
                                        if (masksets[currentActiveMasksetIndex]["lastValidPosition"] >= maskPos)
                                            getActiveMaskSet()['lastValidPosition'] =
                                                -1; //mark mask as validated and invalid
                                        else maskPos = seekNext(pos);
                                    }

                                }
                                if ((getActiveMaskSet()['lastValidPosition'] == undefined && maskPos == seekNext(-1)
                                    ) ||
                    ...
                    $.each(inputValue,
                        function(ndx, charCode) {
                            ...
                            if ((strict && isMask(index)) ||
                                ((charCode != getBufferElement(getActiveBufferTemplate().slice(), index, true) ||
                                        isMask(index)) &&
                    ...
                    $.each(masksets,
                        function(ndx, ms) {
                            if (typeof (ms) == "object") {
                                ...
                                if (ms["lastValidPosition"] != undefined &&
                                    ms["lastValidPosition"] >= highestValidPosition &&
                                    ms["lastValidPosition"] == aml) {
                                    var msComplete = true;
                                    for (var i = 0; i <= aml; i++) {
                ...
                this.mask = function(el) {
                    ...
                    $input.removeClass('focus.inputmask');
                    //bind events
                    $input.closest('form').bind("submit",
                    ...
                    $input.bind("mouseenter.inputmask",
                            function() {
                                var $input = $(this), input = this;
                                ...
                                setTimeout(function() {
                                        ...
                                        if (selectedCaret.begin == selectedCaret.end) {
                                            ...
                                            caret(input,
                                                ...
                                                (isValid(clickPosition, buffer[clickPosition], true) !== false ||
                                                    !isMask(clickPosition))
                                                ? clickPosition
                                                : lastPosition);
                                        }
                                    },
                        ...
                        if (valueProperty && valueProperty.get) {
                            if (!npt._valueGet) {
                                ...
                                npt._valueGet = function() {
                                    return isRTL
                                        ? valueGet.call(this).split('').reverse().join('')
                                        : valueGet.call(this);
                                };
                                    ...
                                    "value",
                                    {
                                        get: function() {
                                            ...
                                            return inputData && inputData['opts'].autoUnmask
                                                ...
                                                : this._valueGet() != masksets[activeMasksetIndex]['_buffer'].join('')
                                                ? this._valueGet()
                                                : '';
                                        },
                                        set: function(value) {
                    ...
                    function
                        shiftR(start, end, c, full) { //full => behave like a push right ~ do not stop on placeholders
                            var buffer = getActiveBuffer();
                            for (var i = start; i <= end && i < getMaskLength(); i++) {
                                if (isMask(i)) {
                                        ...
                                        if (typeof (ms) == "object") {
                                            ...
                                            if (k == opts.keyCode.DELETE) { //handle delete
                                                ...
                                                if (beginPos < maskL) {
                                                    if (opts.isNumeric &&
                                                        opts.radixPoint != "" &&
                                                        buffer[beginPos] == opts.radixPoint) {
                                                        beginPos =
                                                            (buffer.length - 1 == beginPos
                                                            ) /* radixPoint is latest? delete it */
                                                            ? beginPos
                                                            : seekNext(beginPos);
                                                    ...
                                                    if (getActiveMaskSet()['lastValidPosition'] != undefined) {
                                                        if (getActiveMaskSet()['lastValidPosition'] != -1 &&
                                                            getActiveBuffer()[getActiveMaskSet()['lastValidPosition']
                                                            ] ==
                                                            getActiveBufferTemplate()[getActiveMaskSet()[
                                                                'lastValidPosition']])
                                                            getActiveMaskSet()["lastValidPosition"] =
                                                                getActiveMaskSet()["lastValidPosition"] == 0
                                                                ? -1
                                                                : seekPrevious(getActiveMaskSet()["lastValidPosition"]);
                        ...
                        if ((!(e.ctrlKey && e.altKey) && (e.ctrlKey || e.metaKey || ignorable)) && checkval !== true) {
                            ...
                            if (k) {
                                    ...
                                    $.each(masksets,
                                        function(ndx, lmnt) {
                                            if (typeof (lmnt) == "object") {
                                                activeMasksetIndex = ndx;
                                                getActiveMaskSet()["undoBuffer"] =
                                                    getActiveBuffer()
                                                    .join(''); //init undobuffer for recovery when not valid
                                                var posend = pos.end < getMaskLength() ? pos.end : getMaskLength();
                                                if (getActiveMaskSet()["lastValidPosition"] > pos.begin &&
                                                    getActiveMaskSet()["lastValidPosition"] < posend) {
                                ...
                                if (opts.isNumeric && c == opts.radixPoint && checkval !== true) {
                                    ...
                                    if (radixPosition != -1) {
                                        pos.begin =
                                            pos.begin == radixPosition ? seekNext(radixPosition) : radixPosition;
                                        pos.end = pos.begin;
                                        caret(input, pos.begin);
                                    }
                                }

                                var p = (opts.numericInput && strict != true && !isSlctn)
                                    ? seekPrevious(pos.begin)
                                                ...
                                                if (opts.insertMode == true) {
                                                    ...
                                                    while (getBufferElement(bfrClone, lastUnmaskedPosition, true) !=
                                                        ...
                                                        lastUnmaskedPosition =
                                                            lastUnmaskedPosition == 0
                                                            ? -1
                                                            : seekPrevious(lastUnmaskedPosition);
                                                    }
                                                    if (lastUnmaskedPosition >= p) {
                                                        ...
                                                        if (nlvp != getMaskLength() &&
                                                            lvp >= p &&
                                                            (getBufferElement(getActiveBuffer(), nlvp) !=
                                                                getPlaceHolder(nlvp))) {
                                    ...
                                    if (result != undefined) {
                                        ...
                                        if (getActiveMaskSet()["writeOutBuffer"] && result["result"] !== false) {
                                            ...
                                            writeBuffer(input,
                                                ...
                                                ? undefined
                                                : (opts.numericInput
                                                    ? seekPrevious(getActiveMaskSet()["p"])
                                                    : getActiveMaskSet()["p"]));
                                            if (checkval !== true) {
                    ...
                    function keyupEvent(e) {
                        ...
                        caret(input, caretPos.begin, caretPos.end);
                        if (k == opts.keyCode.TAB &&
                            $input.hasClass('focus.inputmask') &&
                            input._valueGet().length == 0 &&
            ...
            'decimal': {
                ...
                getMaskLength:
                    function(buffer,
                        ...
                        repeat,
                        currentBuffer,
                        opts) { //custom getMaskLength to take the groupSeparator into account
                        var calculatedLength = buffer.length;
                        ...
                        var currentBufferStr = currentBuffer.join(''),
                            strippedBufferStr = currentBufferStr.replace(new RegExp(escapedGroupSeparator, "g"), "")
                                .replace(new RegExp(escapedRadixPoint), ""),
                    ...
                    if (e.keyCode == opts.keyCode.TAB) {
                        ...
                        if (radixPosition != -1) {
                            ...
                            for (var i = 1;
                                i <= opts.digits &&
                                    i <
                                    opts.getMaskLength(masksets[activeMasksetIndex]["_buffer"],
                                        masksets[activeMasksetIndex]["greedy"],
                                        masksets[activeMasksetIndex]["repeat"],
                                        buffer,
                                        opts);
                                i++) {
                    ...
                    '~': { //real number
                        validator: function(chrs, buffer, pos, strict, opts) {
                            ...
                            if (!strict &&
                                ...
                                buffer.length == 1) { //handle first char
                                buffer[0] = "";
                                return { "pos": 0 };
                            }
                            ...
                            function validateRegexToken(token, fromGroup) {
                                ...
                                for (var mndx = 0; mndx < token["matches"].length; mndx++) {
                                    ...
                                    if (matchToken["isGroup"] == true) {
                                        ...
                                        var testExp =
                                            regexPart + "{1," + quantifierMax + "}"; //relax quantifier validation
