        ...
        */
        ...
        var core = {
                ...
                $.each(element.data,
                    function(i, entry) {
                        if (i >= element.pageNum * settings.itemsPerPage &&
                            ...
                            entries.push('<div class="row name row' +
                                i +
                                (entry.desc ? '' : ' fn-wide') +
                                '" id="rowheader' +
                                i +
                                ...
                                '">');
                            entries.push('<span class="fn-label' +
            ...
            rightPanel: function(element, leftPanel /* <- never used? */) {
                ...
                case "hours":
                    ...
                    for (var i = 0, len = range.length; i < len; i++) {
                        ...
                        if (rgetDay !== getDay) {
                            var day_class2 = (today - day === 0)
                                ? "today"
                                : tools.isHoliday(day.getTime())
                                ? "holiday"
                                : dowClass[getDay];

                            dayArr.push('<div class="row date ' +
                                day_class2 +
                                '" ' +
                                ' style="width: ' +
            ...
            scrollPanel: function(element, delta) {
                ...
                if (_panelMargin > 0) {
                    element.scrollNavigation.panelMargin = 0;
                    $(element).find(".fn-gantt .dataPanel")
                        .css("margin-left", element.scrollNavigation.panelMargin + "px");
            ...
            synchronizeScroller: function(element) {
                if (settings.navigate === "scroll") {
                    ...
                    pos = pos > 0
                        ? 0
                        : (pos * -1 >= bWidth - (wButton * 0.75))
                        ? (bWidth - (wButton * 1.25)) * -1
                        : pos;
        ...
        var tools = {
            ...
            getMaxDate: function(element) {
                ...
                $.each(element.data,
                    function(i, entry) {
                        $.each(entry.values,
                            function(i, date) {
                                maxDate = maxDate < tools.dateDeserialize(date.to)
                                    ? tools.dateDeserialize(date.to)
                                    : maxDate;
                            });
                    });
            ...
            getMinDate: function(element) {
                ...
                $.each(element.data,
                    function(i, entry) {
                        $.each(entry.values,
                            function(i, date) {
                                minDate = minDate > tools.dateDeserialize(date.from) || minDate === null
            ...
            getProgressBarMargin: function() {
                if (!tools._getProgressBarMargin && tools._getProgressBarMargin !== 0) {
                    ...
                    tools._getProgressBarMargin = parseInt($("#measureBarWidth .fn-gantt .rightPanel .day .bar")
