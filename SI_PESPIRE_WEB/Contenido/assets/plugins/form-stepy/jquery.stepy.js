...
(function($) {
    ...
    var methods = {
            ...
            return this.each(function() {
                ...
                if (self.opt.titleClick) {
                    self.heads.click(function() {
                        var array =
                                self.heads.filter(".stepy-active").attr("id")
                                    .split("-"), // TODO: try keep the number in an attribute.
        ...
        _bindEnter: function() {
            ...
            self.steps.delegate('input[type="text"], input[type="password"]',
                "keypress",
                function(evt) {
                ...
                self.finish.on("click.stepy",
                    function(evt) {
                        if (self.opt.finish && !methods._execute.call(that, self.opt.finish, self.steps.length - 1)) {
                            ...
                            if (!isSubmit &&
                                (!self.opt.validate || methods.validate.call(that, self.steps.length - 1))) {
                                that.submit();
                            }
                        }
                    });
    ...
    $.fn.stepy.defaults = {
        back: undefined,
        backLabel: '<i class="fa fa-angle-left"></i> Back',
        block: false, // WIP...
        description: true,
        duration: undefined,
        enter: true,
