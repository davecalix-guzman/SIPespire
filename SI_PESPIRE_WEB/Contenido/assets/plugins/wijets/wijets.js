...
$.wijets = function() {
    ...
    $.fn.pushWidgetControls = function() {
        ...
        if (controlsContainer.children().length) {
            ...
            controlsHtml.show();
            controlsContainer.append(controlsHtml);
            controlsContainer.slideDown(100,
                "linear",
                function() {
                    controlsContainer.parent().toggleClass("editbox-open");
                });
    ...
    this.registerAction({
        ...
        onClick: function() {
            ...
            $(this).closest("[data-widget]").find(params.target).slideToggle(100,
                "linear",
                function() {
    ...
    this.registerAction({
        ...
        onClick: function() {
            ...
            var controls = button.pushWidgetControls();
            if (controls) {
                controls.find("li span").bind("click",
                    function(e) {
                        var widget = button.closest("[data-widget]");
                        widget.removeClass(
                                "panel-default panel-inverse panel-success panel-green panel-info panel-sky panel-primary panel-midnightblue panel-warning panel-orange panel-danger panel-brown panel-magenta panel-purple panel-indigo panel-grape")
    ...
    this.registerAction({
        ...
        onClick: function() {
            var widget = $(this).closest("[data-widget]");
            bootbox.confirm("Delete this panel?",
                function(choice) {
                    if (choice) {
                        widget.remove();
                    }
