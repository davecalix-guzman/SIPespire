...
!function($) {
    ...
    function UTCToday() {
        var today = new Date();
        return UTCDate(today.getUTCFullYear(),
            today.getUTCMonth(),
            today.getUTCDate(),
            today.getUTCHours(),
            today.getUTCMinutes(),
    ...
    var Datetimepicker = function(element, options) {
        ...
        this.format =
            DPGlobal.parseFormat(options.format ||
        ...
        this.bootcssVer =
            this.isInput
            ? (this.element.is(".form-control") ? 3 : 2)
            : (this.bootcssVer = this.element.is(".input-group") ? 3 : 2);

        this.component = this.element.is(".date")
            ? (this.bootcssVer == 3
                ? this.element
                .find(".input-group-addon .fa-th, .input-group-addon .fa-clock-o, .input-group-addon .fa-calendar")
                .parent()
                : this.element.find(".add-on .icon-th, .add-on .icon-time, .add-on .icon-calendar").parent())
            : false;
        this.componentReset = this.element.is(".date")
            ? (this.bootcssVer == 3
                ? this.element.find(".input-group-addon .fa-times").parent()
        ...
        this.linkFormat =
            DPGlobal.parseFormat(options.linkFormat ||
        ...
        this.weekStart =
            ((options.weekStart || this.element.data("date-weekstart") || dates[this.language].weekStart || 0) % 7);
    ...
    Datetimepicker.prototype = {
        ...
        update: function() {
            ...
            if (arguments && arguments.length && (typeof arguments[0] === "string" || arguments[0] instanceof Date)) {
                ...
                date = this.element.data("date") ||
                    (this.isInput ? this.element.val() : this.element.find("input").val()) ||
                    this.initialDate;
        ...
        fill: function() {
            ...
            for (var i = 0; i < 24; i++) {
                ...
                if (this.showMeridian && dates[this.language].meridiem.length == 2) {
                    ...
                    txt = (i % 12 ? i % 12 : 12);
                    html.push(
                        '<span class="hour' + clsName + " hour_" + (i < 12 ? "am" : "pm") + '">' + txt + "</span>");
                    if (i == 23) {
                        html.push("</fieldset>");
                    }
                } else {
        ...
        showMode: function(dir) {
            ...
            this.picker.find(">div").hide().filter(".datetimepicker-" + DPGlobal.modes[this.viewMode].clsName)
                .css("display", "block");
            this.updateNavArrows();
        },
    ...
    var DPGlobal = {
        ...
        parseDate: function(date, format, language, type) {
            ...
            if (/^[-+]\d+[dmwy]([\s,]+[-+]\d+[dmwy])*$/.test(date)) {
                ...
                return UTCDate(date.getUTCFullYear(),
                    date.getUTCMonth(),
                    date.getUTCDate(),
                    date.getUTCHours(),
                    date.getUTCMinutes(),
            ...
            var parts = date && date.match(this.nonpunctuation) || [],
                ...
                parsed = {},
                setters_order = [
                    "hh", "h", "ii", "i", "ss", "s", "yyyy", "yy", "M", "MM", "m", "mm", "D", "DD", "d", "dd", "H",
                    "HH", "p", "P"
                ],
                setters_map = {
                    hh: function(d, v) {
