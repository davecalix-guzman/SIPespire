    ...
    $.fn.bootstrapDualListbox = function(options) {
        ...
        return this.each(function() {
            ...
            if (settings.bootstrap2compatible) {
                container = $(
                        '<div class="row-fluid bootstrap-duallistbox-container bs2compatible"><div class="span6 box1"><span class="info-container"><span class="info"></span><button type="button" class="btn btn-mini clear1 pull-right">' +
            ...
            function init() {
                ...
                if (settings.nonselectedlistlabel) {
                    ...
                    elements.box1.prepend('<label for="' +
                        nonselectedlistid +
                        '">' +
                ...
                if (settings.selectedlistlabel) {
                    ...
                    elements.box2.prepend('<label for="' +
                        selectedlistid +
                        '">' +
                        settings.selectedlistlabel +
                        '</label>');
                ...
                var height = (elements.originalselect.height() < settings.selectorminimalheight)
                    ? settings.selectorminimalheight
                    : elements.originalselect.height();
            ...
            function bindevents() {
                ...
                elements.originalselect.on('bootstrapduallistbox.refresh',
                    function(e, clearselections) {
                        updateselectionstates();
                ...
                elements.filter1clear.on('click',
                    function() {
                        elements.filterinput1.val('');
                        refreshselects();
                    });

                elements.filter2clear.on('click',
                    function() {
                        elements.filterinput2.val('');
                        refreshselects();
                    });

                elements.movebutton.on('click',
                    function() {
                        move();
                    });

                elements.moveallbutton.on('click',
                    function() {
                        moveall();
                    });

                elements.removebutton.on('click',
                    function() {
                        remove();
                    });

                elements.removeallbutton.on('click',
                    function() {
                        removeall();
                    });

                elements.filterinput1.on('change keyup',
                    function() {
                        filter1();
                    });

                elements.filterinput2.on('change keyup',
                    function() {
                        filter2();
                    });
                ...
                if (settings.moveonselect) {
                    ...
                    elements.select1.on('change',
                        function() {
                            move();
                        });
                    elements.select2.on('change',
                        function() {
                            remove();
                        });
                }
