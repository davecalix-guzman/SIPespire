        ...
        jQuery.event.special.submit = {
            setup: function(data, namespaces, fn) {
                if (this.nodeName.toLowerCase() !== "form") {
                    ...
                    jQuery.event.add(this,
                        "keypress.specialSubmit." + fn.guid,
                        function(e) {
                            var elem = e.target, type = elem.type;
    ...
    function liveHandler(event) {
        ...
        for (j in live) {
            ...
            if (fn.live === event.type ||
                ...
                data = fn.data;
                if (!(data.beforeFilter &&
                    data.beforeFilter[event.type] &&
...
//   Thus far that includes Google Chrome.
            ...
            if (toString.call(checkSet) === "[object Array]") {
                if (!prune) {
                    ...
                    for (var i = 0; checkSet[i] != null; i++) {
                        if (checkSet[i] &&
                            (checkSet[i] === true || checkSet[i].nodeType === 1 && contains(context, checkSet[i]))) {
                            results.push(set[i]);
                        }
                    }
                } else {
        ...
        for (var type in Expr.match) {
            ...
            Expr.leftMatch[type] = new RegExp(/(^(?:.|\r|\n)*?)/.source +
                Expr.match[type].source.replace(/\\(\d+)/g,
                    function(all, num) {
                        return "\\" + (num - 0 + 1);
                    }));
        }
                    ...
                    if (scripts &&
                        ...
                        if (ret[i].nodeType === 1) {
                            ret.splice.apply(ret,
                                [i + 1, 0].concat(jQuery.makeArray(ret[i].getElementsByTagName("script"))));
                        }
        ...
        ajax: function(origSettings) {
            ...
            try {
                ...
                xhr.setRequestHeader("Accept",
                    s.dataType && s.accepts[s.dataType] ? s.accepts[s.dataType] + ", */*" : s.accepts._default);
            } catch (e) {
            }
        ...
        jQuery.fn.offset = function(options) {
            ...
            var box = elem.getBoundingClientRect(),
                ...
                top = box.top +
                    ...
                    clientTop,
                left = box.left +
                    (self.pageXOffset || jQuery.support.boxModel && docElem.scrollLeft || body.scrollLeft) -
                    clientLeft;
            ...
            while ((elem = elem.parentNode) && elem !== body && elem !== docElem) {
                ...
                if (elem === offsetParent) {
                    ...
                    if (jQuery.offset.doesNotAddBorder &&
                        !(jQuery.offset.doesAddBorderForTableAndCells && /^t(able|d|h)$/i.test(elem.nodeName))) {
