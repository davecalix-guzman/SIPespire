...
(function($) {
    ...
    $.fn.mapael = function(options) {
        ...
        return this.each(function() {
            ...
            $self.on("zoom",
                function(e, level, x, y) {
                    ...
                    if (currentLevel == 0) {
                        ...
                        paper.setViewBox(
                            Math.min(Math.max(0, x - (mapConf.width / (1 + currentLevel * options.map.zoom.step)) / 2),
                                (mapConf.width - (mapConf.width / (1 + currentLevel * options.map.zoom.step)))),
                            Math.min(Math.max(0, y - (mapConf.height / (1 + currentLevel * options.map.zoom.step)) / 2),
                                (mapConf.height - (mapConf.height / (1 + currentLevel * options.map.zoom.step)))),
            ...
            if (typeof options.map.zoom.init != "undefined") {
                ...
                } else if (typeof options.map.zoom.init.x != "undefined" &&
                    typeof options.map.zoom.init.y != "undefined") {
            ...
            */
            $self.on("update",
                function(e, updatedOptions, newPlots, deletedPlots, opt) {
                    var i = 0,
                        id = 0,
                        animDuration = 0,
                        elemOptions = {},
                    ...
                    if (typeof deletedPlots == "object") {
                        for (; i < deletedPlots.length; i++) {
                            if (typeof plots[deletedPlots[i]] != "undefined") {
                                if (animDuration > 0) {
                                    (function(plot) {
                                        plot.mapElem.animate({ 'opacity': 0 },
                                            animDuration,
                                            "linear",
                                            function() { plot.mapElem.remove(); });
                                        if (plot.textElem) {
                                            plot.textElem.animate({ 'opacity': 0 },
                                                animDuration,
                                                "linear",
                                                function() { plot.textElem.remove(); });
                                        }
                            ...
                            if (typeof plots[id] == "undefined") {
                                ...
                                if (animDuration > 0) {
                                    ...
                                    plots[id].mapElem.animate({
                                            'opacity': (typeof plots[id].mapElem.originalAttrs.opacity != "undefined")
                                                ? plots[id].mapElem.originalAttrs.opacity
                                                : 1
                                        },
                                        animDuration);
                                    plots[id].textElem.animate({
                                            'opacity': (typeof plots[id].textElem.originalAttrs.opacity != "undefined")
                                                ? plots[id].textElem.originalAttrs.opacity
                                                : 1
                                        },
                                        animDuration);
                                }
                    ...
                    for (id in plots) {
                        ...
                        if ("square" == elemOptions.type) {
                            ...
                            elemOptions.attrs.x = plots[id].mapElem.attrs.x -
                                (elemOptions.size - plots[id].mapElem.attrs.width) / 2;
                            elemOptions.attrs.y = plots[id].mapElem.attrs.y -
                                (elemOptions.size - plots[id].mapElem.attrs.height) / 2;
                        } else { // Default : circle
            ...
            if (options.map.width) {
                ...
                if (options.legend.plot.slices && options.legend.plot.display)
                    plotLegend = $.fn.mapael.createLegend($self,
                        options,
                        "plot",
                        plots,
                        (options.map.width / mapConf.width));
            } else {
                $(window).on("resize",
                    function() {
                ...
                var createPlotLegend = function() {
                    if (options.legend.plot.slices && options.legend.plot.display)
                        plotLegend = $.fn.mapael.createLegend($self,
                            ...
                            "plot",
                            plots,
                            ($container.width() / mapConf.width));

                    $container.unbind("resizeEnd", createPlotLegend);
                };

                $container.on("resizeEnd",
                    function() {
	...
	*/
    $.fn.mapael.updateElem = function(elemOptions, elem, $tooltip, animDuration) {
        ...
        // Update text
        if (elem.textElem) {
            if (typeof elemOptions.text != "undefined" &&
                typeof elemOptions.text.content != "undefined" &&
                elemOptions.text.content != elem.textElem.attrs.text)
	...
	*/
    $.fn.mapael.setTooltip = function(elem, $tooltip) {
        var tooltipTO = 0, $container = $tooltip.parent(), containerY2 = $container.offset().left + $container.width();
            ...
            function(e) {
                tooltipTO = setTimeout(
                    function() {
                        ...
                        $tooltip.css({
                            "left": Math.min(containerY2 - $tooltip.outerWidth() - 5, e.pageX + 12),
                            "top": e.pageY + 23 - $(window).scrollTop()
                        });
                    },
                    120
                );
    ...
    */
    $.fn.mapael.setEventHandlers = function(id, elemOptions, mapElem, textElem) {
        for (var event in elemOptions.eventHandlers) {
            (function(event) {
                $(mapElem.node).on(event,
                    function(e) { !$.fn.mapael.panning && elemOptions.eventHandlers[event](e, id, mapElem, textElem) });
                textElem &&
                    $(textElem.node).on(event,
                        function(e) {
                            !$.fn.mapael.panning && elemOptions.eventHandlers[event](e, id, mapElem, textElem)
                        });
            })(event);
        }
    };
        ...
        $container.on("mousedown",
            ...
            function(e) {
                ...
                if (mousedown && currentLevel != 0) {
                    var offsetX =
                            (previousX - e.pageX) / (1 + (currentLevel * options.step)) * (mapWidth / paper.width),
                        offsetY = (previousY - e.pageY) /
                            (1 + (currentLevel * options.step)) *
                            (mapHeight / paper.height);
	...
	*/
    $.fn.mapael.createLegend = function($container, options, legendType, elems, scale) {
        var legendOptions = options.legend[legendType],
            $legend =
                (legendType == "plot")
                    ? $("." + options.legend.plot.cssClass, $container).empty()
                    : $("." + options.legend.area.cssClass, $container).empty(),
                    ...
                    (function(i, elem, label) {
                        $(label.node).on("click",
                            function() {
                                ...
                                for (var id in elems) {
                                    if ((typeof legendOptions.slices[i].min == "undefined" ||
                                            elems[id].value >= legendOptions.slices[i].min) &&
                                        (typeof legendOptions.slices[i].max == "undefined" ||
                                            elems[id].value < legendOptions.slices[i].max)
                                    ) {
                                        (function(id) {
                                            if (!label.hidden) {
                                                elems[id].mapElem.animate(
                                                    ...
                                                    "linear",
                                                    function() {
                                                        (legendOptions.hideElemsOnClick.opacity == 0) &&
                                                            elems[id].mapElem.hide();
                                                    });
                                                elems[id].textElem &&
                                                    elems[id].textElem.animate(
                                                        { 'opacity': legendOptions.hideElemsOnClick.opacity },
                                                        300,
                                                        "linear",
                                                        function() {
                                                            (legendOptions.hideElemsOnClick.opacity == 0) &&
                                                                elems[id].textElem.hide();
                                                        });
                                            } else {
                                                ...
                                                elems[id].mapElem.animate({
                                                        'opacity': typeof elems[id].mapElem.originalAttrs.opacity !=
                                                            "undefined"
                                                            ? elems[id].mapElem.originalAttrs.opacity
                                                            : 1
                                                    },
                                                    300);
                                                elems[id].textElem &&
                                                    elems[id].textElem.animate({
                                                            'opacity': typeof elems[id].textElem.originalAttrs
                                                                .opacity !=
                                                                "undefined"
                                                                ? elems[id].textElem.originalAttrs.opacity
                                                                : 1
                                                        },
                                                        300);
                                            }
    ...
    */
    $.fn.mapael.setHover = function(paper, mapElem, textElem) {
        var $mapElem = {},
            ...
            overBehaviour =
                function() {
                    hoverTO = setTimeout(function() { $.fn.mapael.elemHover(paper, mapElem, textElem); }, 120);
                },
	...
	*/
    $.fn.mapael.getTextPosition = function(bbox, textPosition, margin) {
        var textX = 0, textY = 0, textAnchor = "";
