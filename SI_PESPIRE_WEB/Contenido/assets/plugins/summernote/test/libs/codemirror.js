...
window.CodeMirror = (function() {
    ...
    function makeDisplay(place, docStart) {
        ...
        var input = d.input = elt("textarea",
            null,
            null,
        ...
        d.wrapper = elt("div",
            [
                d.inputDiv, d.scrollbarH, d.scrollbarV,
    ...
    function updateDisplayInner(cm, changes, visible, forced) {
        ...
        var different = from != display.showingFrom ||
    ...
    function buildLineElement(cm, line, lineNo, dims, reuse) {
        ...
        if (cm.options.lineNumbers || markers) {
            var gutterWrap = wrap.insertBefore(elt("div",
                    null,
                    null,
    ...
    function updateSelection(cm) {
        ...
        if (cm.options.moveInputWithCursor) {
            ...
            display.inputDiv.style.top = Math.max(0,
                    Math.min(display.wrapper.clientHeight - 10,
                        headPos.top + lineOff.top - wrapOff.top)) +
                "px";
            display.inputDiv.style.left = Math.max(0,
    ...
    function updateSelectionRange(cm) {
        ...
        function drawForLine(line, fromArg, toArg) {
            ...
            iterateBidiSections(getOrder(lineObj),
                fromArg || 0,
    ...
    function paddingLeft(display) {
        var e = removeChildrenAndAdd(display.measure, elt("pre", null, null, "text-align: left"))
    ...
    function endOperation(cm) {
        ...
        if (display.maxLineChanged && !cm.options.lineWrapping && display.maxLine) {
            ...
            var maxScrollLeft = Math.max(0,
                display.sizer.offsetLeft + display.sizer.offsetWidth - display.scroller.clientWidth);
        ...
        if (op.updateScrollPos) {
            var top = Math.max(0,
                Math.min(display.scroller.scrollHeight - display.scroller.clientHeight, newScrollPos.scrollTop));
            var left = Math.max(0,
                Math.min(display.scroller.scrollWidth - display.scroller.clientWidth, newScrollPos.scrollLeft));
    ...
    function posFromMouse(cm, e, liberal) {
        ...
        if (!liberal) {
            ...
            if (target == display.scrollbarH ||
                ...
                target == display.gutterFiller) return null;
        }
            ...
            var loadFile = function(file, i) {
                ...
                reader.onload = function() {
                    ...
                    if (++read == n) {
                        ...
                        makeChange(cm.doc,
                            { from: pos, to: pos, text: splitLines(text.join("\n")), origin: "paste" },
                            "around");
                    }
                };
    ...
    function onScrollWheel(cm, e) {
        ...
        if (dx && !gecko && !opera && wheelPixelsPerUnit != null) {
            if (dy)
                setScrollTop(cm,
                    Math.max(0,
                        Math.min(scroll.scrollTop + dy * wheelPixelsPerUnit,
                            scroll.scrollHeight - scroll.clientHeight)));
            setScrollLeft(cm,
                Math.max(0,
                    Math.min(scroll.scrollLeft + dx * wheelPixelsPerUnit, scroll.scrollWidth - scroll.clientWidth)));
            e_preventDefault(e);
    ...
    function addToScrollPos(cm, left, top) {
        var pos = cm.curOp.updateScrollPos ||
            (cm.curOp.updateScrollPos = { scrollLeft: cm.doc.scrollLeft, scrollTop: cm.doc.scrollTop });
    ...
    StringStream.prototype = {
        ...
        column: function() {
            if (this.lastColumnPos < this.start) {
                this.lastColumnValue = countColumn(this.string,
                    this.start,
                    this.tabSize,
                    this.lastColumnPos,
    ...
    function markedSpansAfter(old, endCh, isInsert) {
        if (old)
            for (var i = 0, nw; i < old.length; ++i) {
                ...
                if (endsAfter ||
                    marker.type == "bookmark" && span.from == endCh && (!isInsert || span.marker.insertLeft)) {
                    var startsBefore = span.from == null ||
    ...
    function buildLineContent(cm, realLine, measure, copyWidgets) {
        ...
        if (measure && (ie || ie_gt10) && (order = getOrder(line))) {
            ...
            if (last.from + 1 == last.to && prev && last.level < prev.level) {
                ...
                if (span)
                    span.parentNode.insertBefore(span.measureRight = zeroWidthElement(cm.display.measure),
                        span.nextSibling);
            }
        }
    ...
    var Doc = CodeMirror.Doc = function(text, mode, firstLine) {
        ...
        this.sel =
            { from: start, to: start, head: start, anchor: start, shift: false, extend: false, goalColumn: null };
        this.id = ++nextDocId;
    ...
    Doc.prototype = createObj(BranchChunk.prototype,
        {
            ...
            replaceSelection: function(code, collapse, origin) {
                makeChange(this,
                    { from: this.sel.from, to: this.sel.to, text: splitLines(code), origin: origin },
                    collapse || "around");
            },
    ...
    function attachLocalSpans(doc, change, from, to) {
        ...
        doc.iter(Math.max(doc.first, from),
    ...
    function historyChangeFromChange(doc, change) {
        ...
        linkedDocs(doc,
            function(doc) { attachLocalSpans(doc, histChange, change.from.line, change.to.line + 1); },
            true);
        return histChange;
    }
    ...
    var isExtendingChar =
        /[\u0300-\u036F\u0483-\u0487\u0488-\u0489\u0591-\u05BD\u05BF\u05C1-\u05C2\u05C4-\u05C5\u05C7\u0610-\u061A\u064B-\u065F\u0670\u06D6-\u06DC\u06DF-\u06E4\u06E7-\u06E8\u06EA-\u06ED\uA66F\u1DC0–\u1DFF\u20D0–\u20FF\uA670-\uA672\uA674-\uA67D\uA69F\udc00-\udfff\uFE20–\uFE2F]/;
