...
window.ParsleyExtend = $.extend(window.ParsleyExtend,
    {
        ...
        eventValidate: function(event) {
            ...
            if (new RegExp("key").test(event.type))
                if (!this._ui.validationInformationVisible &&
        ...
        _remote: function(deferred) {
            var
                ...
                validator = this.options.remoteValidator ||
                    Â(true === this.options.remoteReverse ? "reverse" : "default");
...
*/
    (function(exports) {
        ...
        */
        ...
        ? this["undefined" !== typeof validatorjs_ns ? validatorjs_ns : "Validator"] = {}
        : exports);
                ...
                return $.extend(new Validator.Assert().Range(array[0], array[1]),
                    {
                        ...
                        requirementsTransformer: function() {
                            for (var i = 0; i < array.length; i++)
                                array[i] = "string" === typeof array[i] && !isNaN(array[i])
                                    ? parseInt(array[i], 10)
                                    : array[i];
                            return array;
                        }
    ...
    ParsleyUI.prototype = {
        ...
        focus: function(formInstance) {
            ...
            for (var i = 0; i < formInstance.fields.length; i++)
                if (true !== formInstance.fields[i].validationResult &&
        ...
        _getErrorMessage: function(fieldInstance, constraint) {
            ...
            if ("undefined" !== typeof fieldInstance.options[customConstraintErrorMessage])
                return window.ParsleyValidator.formatMessage(fieldInstance.options[customConstraintErrorMessage],
        ...
        setupField: function(fieldInstance) {
            ...
            _ui.errorsWrapperId = "parsley-id-" +
        ...
        _insertErrorWrapper: function(fieldInstance) {
            ...
            if ("string" === typeof fieldInstance.options.errorsContainer)
                if ($(fieldInstance.options.errorsContainer + "").length)
                    ...
                    window.console.warn("The errors container `" +
            ...
            return "undefined" === typeof fieldInstance.options.multiple
                ...
                : fieldInstance.$element.parent().after(fieldInstance._ui.$errorsWrapper);
        },
        actualizeTriggers: function(fieldInstance) {
            var that = this;
            // Remove Parsley events already binded on this field
            if (fieldInstance.options.multiple)
                $("[" + fieldInstance.options.namespace + 'multiple="' + fieldInstance.options.multiple + '"]').each(
                    function() {
                        $(this).off(".Parsley");
                    });
            else
                ...
                $("[" + fieldInstance.options.namespace + 'multiple="' + fieldInstance.options.multiple + '"]').each(
                    function() {
                        $(this).on(
                            ...
                            false,
                            $.proxy("function" === typeof fieldInstance.eventValidate
        ...
        manageFailingFieldTrigger: function(fieldInstance) {
                ...
                $("[" + fieldInstance.options.namespace + 'multiple="' + fieldInstance.options.multiple + '"]').each(
                    function() {
                        if (!new RegExp("change", "i").test($(this).parsley().options.trigger || ""))
                            return $(this).on("change.ParsleyFailedOnce",
                                false,
            ...
            if (fieldInstance.$element.is("select"))
                if (!new RegExp("change", "i").test(fieldInstance.options.trigger || ""))
                    return fieldInstance.$element.on("change.ParsleyFailedOnce",
                        false,
                        $.proxy(fieldInstance.validate, fieldInstance));
            // All other inputs fields
            if (!new RegExp("keyup", "i").test(fieldInstance.options.trigger || ""))
                return fieldInstance.$element.on("keyup.ParsleyFailedOnce",
                    false,
                    $.proxy(fieldInstance.validate, fieldInstance));
        },
        ...
        _errorClass: function(fieldInstance) {
            fieldInstance._ui.validationInformationVisible = true;
            fieldInstance._ui.$errorClassHandler.removeClass(fieldInstance.options.successClass)
    ...
    ParsleyForm.prototype = {
        ...
        _bindFields: function() {
            ...
            this.$element.find(this.options.inputs).each(function() {
                ...
                if (("ParsleyField" === fieldInstance.__class__ || "ParsleyFieldMultiple" === fieldInstance.__class__
                    ) &&
                    !fieldInstance.$element.is(fieldInstance.options.excluded))
                    if ("undefined" ===
                        typeof self.fieldsMappedById[fieldInstance.__class__ + "-" + fieldInstance.__id__]) {
        ...
        return $.extend(window.ParsleyValidator.validators[name](requirements),
            {
                ...
                isDomConstraint: isDomConstraint ||
                    ParsleyUtils.attr(parsleyField.$element, parsleyField.options.namespace, name)
            });
    };
    ...
    ParsleyField.prototype = {
        ...
        isValid: function(force, value) {
            ...
            for (var i = 0; i < priorities.length; i++)
                if (true !==
                    (this.validationResult = this.validateThroughValidator(value, this.constraints, priorities[i])))
                    return false;
            return true;
        },
        ...
        refreshConstraints: function() {
            ...
            for (var i = 0; i < this.$elements.length; i++) {
                ...
                for (var j = 0; j < fieldConstraints.length; j++)
                    this.addConstraint(fieldConstraints[j].name,
        ...
        for (var i = 0; i < subscribed[name].length; i++) {
            ...
            if ("undefined" === typeof subscribed[name][i].instance) {
                subscribed[name][i].fn.apply(
                    "undefined" !== typeof subscribed[name][i].ctxt ? subscribed[name][i].ctxt : o,
...
//     Parsley may be freely distributed under the MIT license.
    ...
    Parsley.prototype = {
        init: function($element, options, parsleyFormInstance) {
            ...
            this.OptionsFactory = new ParsleyOptionsFactory(ParsleyDefaults,
        ...
        isMultiple: function() {
            return (this.$element.is("input[type=radio], input[type=checkbox]") &&
        ...
        handleMultiple: function(parsleyFormInstance) {
            ...
            this.options = $.extend(this.options,
                parsleyFormInstance ? parsleyFormInstance.OptionsFactory.get(parsleyFormInstance) : {},
            ...
            if ($("[" + this.options.namespace + "multiple=" + multiple + "]").length)
                for (var i = 0; i < $("[" + this.options.namespace + "multiple=" + multiple + "]").length; i++)
                    if ("undefined" !==
                        typeof $($("[" + this.options.namespace + "multiple=" + multiple + "]").get(i))
                        .data("Parsley")) {
                        parsleyMultipleInstance =
                            $($("[" + this.options.namespace + "multiple=" + multiple + "]").get(i)).data("Parsley");
