...
(function($) {
    ...
    $(function() {
        var search = function(str) {
                var tmp = str && str.length
                        ? $.vakata.search(str, true, { threshold: 0.2, fuzzy: true, caseSensitive: false })
                        : null,
                    res = $('#api_inner')
                        .find('.item').hide()
                        .filter(function() {
            ...
            filter = function(str) {
                ...
                if (str) {
                    ...
                    if (i && i.length) {
                        ...
                        if (i.offset().top < $(document).scrollTop() ||
                            i.offset().top + i.height() > $(document).scrollTop() + $(window).height()) {
                            i[0].scrollIntoView();
                        }
                    }
                }
        ...
        $(window).resize(function() {
            ...
            to1 = setTimeout(function() {
                    $('.page').css('minHeight', '0px')
                        .css('minHeight', ($(document).height() - $('#head').outerHeight()) + 'px');
                },
                50);
        });
        ...
        $.address
            ...
            .init(function(e) {
                $('a:not([href^=http])').not($('.demo a')).address().on('click',
                    function() {
                        if ($.address.pathNames().length < 2 && !$.address.parameter('f')) {
            ...
            .change(function(e) {
                ...
                $('#menu').find('a[href$="' + page + '"]').blur().parent().addClass('active').siblings()
                    .removeClass('active');
                ...
                if (page === 'api') {
                    ...
                    if (elem.length) {
                        if (elem.hasClass('tab-content-item')) {
                            elem.siblings().hide().end().show().parent().prev().children().removeClass('active')
                                .eq(elem.index()).addClass('active');
                        } else {
        ...
        $.getJSON('./jstree.json',
            function(data) {
                ...
                $.each(data,
                    function(ii, v) {
                        if (v.description.full.indexOf('<p>lobals') === 0) {
                            return true;
                        }
                        if (v.ignore) {
                            return true;
                        ...
                        for (i = 0, j = v.tags.length; i < j; i++) {
                            ...
                            case 'return':
                                retrn =
                                    '<ul class="params list-unstyled"><li><code class="param return">Returns</code><p><code class="type">' +
                                    v.tags[i].types.join('</code> <code class="type">') +
                                    '</code> ' +
                                ...
                                break;
                            case 'param':
                                params.push('<code class="param">' +
                                    v.tags[i].name +
                                    '</code><p><code class="type">' +
                                    v.tags[i].types.join('</code> <code class="type">') +
                                    '</code> ' +
                                    v.tags[i].description +
                                    '</p>');
                        ...
                        if (name) {
                            ...
                            str += '<h4><code class="' +
                                (name.indexOf('(') === -1 ? (evnt ? 'evnt' : 'prop') : 'func') +
                                '">' +
                                name +
                                (evnt ? ' Event <i class="glyphicon glyphicon-flash"></i>' : '') +
                                '</code>';
                            if (plugin) {
                                str += '<code class="meta plugin"><i class="glyphicon glyphicon-leaf"></i> ' +
                                    plugin +
                                    ' plugin</code> ';
                            }
                            if (priv) {
                                str += '<code class="meta">private</code> ';
                            }
                            str += '</h4>';
                        }
                        ...
                        if (trig) {
                            str +=
                                '<ul class="params list-unstyled"><li><code class="param trigger">Triggers</code><p><code class="evnt">' +
                                trig.split(',').join('</code> <code class="evnt">') +
                                '</code></p></li></ul>';
                        }
                        str += '</div>';
                        str += '</div>';
                ...
                $('#api h3').prepend('<i class="glyphicon glyphicon-leaf"></i>&nbsp;').closest('.item').css({
                    'background': 'white',
                    'border': '0',
                    'borderRadius': '0', /*'borderBottom' : '1px solid #8b0000', 'textAlign' : 'center',*/
                    'marginTop': '0',
                    'paddingTop': '0'
                }).prev().css('marginBottom', '3em');
