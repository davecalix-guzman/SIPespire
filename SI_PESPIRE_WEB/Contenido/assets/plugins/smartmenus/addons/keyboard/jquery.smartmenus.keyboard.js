            ...
            docKeydown: function(e) {
                ...
                case 38: // Up
                    if (obj.isCollapsible()) {
                        ...
                        if (level == 1 &&
                            ...
                            obj.visibleSubMenus[level] &&
                            obj.opts.bottomToTopSubMenus) {
                ...
                case 40: // Down
                    if (obj.isCollapsible()) {
                        ...
                        if (obj.visibleSubMenus[level] &&
                            !obj.visibleSubMenus[level].hasClass("mega-menu") &&
                            ($firstSubItem = getFirstItemLink(obj.visibleSubMenus[level])).length) {
                            $firstSubItem.focusSM();
                            // if this is the last item of a sub menu, move to the next parent item
                        } else if (level > 1 &&
                            ($lastItem = getLastItemLink($ul)).length &&
                            $target[0] == $lastItem[0]) {
    ...
    $.extend($.SmartMenus.prototype,
        {
            ...
            keyboardSetHotkey: function(keyCode, modifiers) {
                var self = this;
                $(document).bind("keydown.smartmenus" + this.rootId,
                    function(e) {
                        if (keyCode == e.keyCode) {
                            ...
                            if (modifiers) {
                                ...
                                $.each(["ctrlKey", "shiftKey", "altKey", "metaKey"],
                                    function(index, value) {
                                        if ($.inArray(value, modifiers) >= 0 && !e[value] ||
                                            $.inArray(value, modifiers) < 0 && e[value]) {
                                            procede = false;
                                            return false;
                                        }
                                    });
